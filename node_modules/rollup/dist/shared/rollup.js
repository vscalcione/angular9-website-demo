...
function es(magicString, { intro, outro, dependencies, exports }, options) {
    const _ = options.compact ? '' : ' ';
    const n = options.compact ? '' : '\n';
    ...
    getDependenciesToBeIncluded() {
        ...
        if (this.isEntryPoint || this.dynamicallyImportedBy.length > 0 || this.graph.preserveModules) {
            for (const exportName of [...this.getReexports(), ...this.getExports()]) {
                relevantDependencies.add(this.getVariableForExportName(exportName).module);
            }
        }
...
class Chunk$1 {
    ...
    getRelativePath(targetPath) {
        const relativePath = normalize(relative(path.dirname(this.id), targetPath));
...
class ModuleLoader {
    ...
    async resolveId(source, importer, skip) {
        return this.normalizeResolveIdResult(this.isExternal(source, importer, false)
            ? false
            : await this.pluginDriver.hookFirst('resolveId', [source, importer], null, skip), importer, source);
    }
...
class PluginDriver {
    constructor(graph, userPlugins, pluginCache, preserveSymlinks, basePluginDriver) {
...
class Graph {
    constructor(options, watcher) {
        ...
        this.pluginDriver = new PluginDriver(this, options.plugins, this.pluginCache, options.preserveSymlinks === true);
        if (watcher) {
